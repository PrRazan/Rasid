...
class _MalfunctionsScreenState extends State<MalfunctionsScreen> {
  ...
  final List<Malfunction> _camryMalfunctions = [
    Malfunction(
      ...
      type: "التكييف",
      faultDate: "2024 - 8 - 10",
      description: "نقص في غاز الفريون",
      fixDate: "2024 - 8 - 12",
    ),
    Malfunction(
      title: "عطل في البطارية",
      ...
      type: "البطارية",
      faultDate: "2024 - 1 - 15",
      description: "ضعف في جهد البطارية",
      fixDate: "2024 - 1 - 16",
    ),
    Malfunction(
      title: "عطل في المحرك",
      ...
      type: "المحرك",
      faultDate: "2024 - 3 - 10",
      description: "تغيير شمعات الاحتراق",
      fixDate: "2024 - 3 - 12",
    ),
  ];

  final List<Malfunction> _fordMalfunctions = [
    Malfunction(
      title: "عطل في المساعدات",
      status: "لم يتم إصلاحه",
      isFixed: false,
      type: "المساعدات",
      faultDate: "2025 - 12 - 05",
      description: "تسريب زيت في المساعد الأمامي الأيمن",
    ),
  ];

  final List<Malfunction> _yarisMalfunctions = [
    Malfunction(
      title: "عطل في البطارية",
      status: "لم يتم إصلاحه",
      isFixed: false,
  ...
  List<Malfunction> _currentCarMalfunctions = [];
  ...
  void _initializeCarData() {
    if (widget.carModel.contains("كامري")) {
      _currentCarMalfunctions = List.from(_camryMalfunctions);
    } else if (widget.carModel.contains("فورد")) {
      _currentCarMalfunctions = List.from(_fordMalfunctions);
    } else if (widget.carModel.contains("يارس")) {
      _currentCarMalfunctions = List.from(_yarisMalfunctions);
    } else {
      _currentCarMalfunctions = List.from(_allMalfunctions);
    }
    _filteredMalfunctions = List.from(_currentCarMalfunctions);
  }

  @override
  void didUpdateWidget(MalfunctionsScreen oldWidget) {
  ...
  void _applyFilters() {
    setState(() {
      _filteredMalfunctions = _currentCarMalfunctions.where((m) {
  ...
  Widget build(BuildContext context) {
    final currentCount = _currentCarMalfunctions.where((m) => !m.isFixed).length;
    final fixedCount = _currentCarMalfunctions.where((m) => m.isFixed).length;
